...
local function play_effect(ply, fraction, origin, normal)
    ply:EmitSound(("weapons/fx/rics/ric%s.wav"):format(math.random(5)), 75, math.Clamp(300 * fraction, 70, 150), math.Clamp(fraction * 0.5, 0, 1))
    ply:EmitSound(("physics/plastic/plastic_barrel_impact_bullet3.wav"):format(math.random(5)), 75, 100, math.Clamp(fraction, 0, 1))
    local ef = EffectData()
    ef:SetOrigin(origin)
...
if SERVER then
    hook.Add("GetFallDamage", hook_key, function(ply, speed)
        if ply:KeyDown(IN_JUMP) then return 0 end
    end)
end
...
hook.Add("Move", hook_key, function(ply, data)
    ...
    if data:KeyPressed(IN_JUMP) or data:KeyWasDown(IN_JUMP) then
        ...
        if not ply:IsOnGround() and res.Hit and math.abs(res.HitNormal.z) < 0.7 and (not ply.bhop_wall_bounce_origin or ply.bhop_wall_bounce_origin:Distance(data:GetOrigin()) > ply:BoundingRadius()) and velocity.x ~= 0 and velocity.y ~= 0 then
...
hook.Add("PostGamemodeLoaded", hook_key, function()
    function GAMEMODE:StartMove()
    end

    function GAMEMODE:FinishMove()
    end
